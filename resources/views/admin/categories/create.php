            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select id="parent_id" name="parent_id" 
                        ...
                        <?php foreach ($parentCategories as $category): ?>
                            <option value="<?= $category->getKey() ?>" <?= $view->old('parent_id') == $category->getKey() ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-orange"><?= $view->e($view->old('description')) ?></textarea>
                </div>
                
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-1">Image URL</label>
                    <input type="url" id="image" name="image" 
                           value="<?= $view->e($view->old('image')) ?>"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="display_order" name="display_order" min="0"
                               value="<?= $view->e($view->old('display_order', '0')) ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-orange">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" 
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-orange">
                            <option value="active" <?= $view->old('status') === 'active' ? 'selected' : '' ?>>Active</option>
